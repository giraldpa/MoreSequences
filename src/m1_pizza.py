...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    expected = [rg.Point(219.0, 40.0),  # All numbers are approximate.
                rg.Point(204.6, 74.6),
                rg.Point(170.0, 89.0),
                rg.Point(135.4, 74.6),
                rg.Point(121.0, 40.0),
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    title = 'PIZZA test 4: 345 slices, thin black lines'
    ...
    pizza(window, circle, 345, 'black', 2)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    list_starts = generate_points_on_circle(circle, number_of_slices)
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON test 3: 4 segments with thick pink lines'
    window = rg.RoseWindow(500, 500, title)
    circle = rg.Circle(rg.Point(200, 300), 95)
    circle.outline_thickness = 4
    polygon(window, circle, 4, 'pink', 8)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    circle.attach_to(window)
    poly_points = generate_points_on_circle(circle, number_of_segments)
    for k in range(len(poly_points)):
        start = poly_points[k]
        end = poly_points[k - 1]
        line = rg.Line(start, end)
        line.color = color
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'FANCY POLYGON test 4: 23 magenta lines on yellow circle, hops = 6'
    window = rg.RoseWindow(500, 500, title)
    circle = rg.Circle(rg.Point(250, 200), 100)
    circle.fill_color = 'yellow'
    fancy_polygon(window, circle, 21, 6, 'magenta', 3)
    window.close_on_mouse_click()
